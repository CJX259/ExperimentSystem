...
const PageLayout: React.FC<PageLayout> = ({ user, loginByCookie, logout, children, location, route, history, match }: IRouteComponentProps) => {
  // 登录则不要渲染
  ...
  return <div className="layout">
    <Layout style={{ minHeight: '100vh' }}>
      <Sider theme="light" collapsible collapsed={collapsed} onCollapse={onCollapse}>
        ...
        <Menu style={{ userSelect: 'none' }} theme="light" mode="inline" defaultSelectedKeys={isDefault(location)} defaultOpenKeys={['sub1']}>
          <Menu.Item key="2" icon={<DesktopOutlined />}>
            <Link to="/addcourse">添加课程</Link>
          </Menu.Item>
          <SubMenu key="sub1" icon={<TableOutlined />} title="查看课程" >
            {courses.map((course: Course) => {
              return <Menu.Item key={'sub1_' + course.id}>
                <Link to={{
      ...
      <Layout className={styles['site-layout']}>
        <Header className={styles["site-layout-background"]} style={{ padding: 0 }} >
          <span className={styles.header}>广州大学实验报告上传系统-教师端</span>
          <Popconfirm
            title="确认登出吗"
        ...
        <Footer style={{ textAlign: 'center' }}>广州大学实验报告上传系统  DESIGNED BY 638 TEAM</Footer>
      </Layout>
    </Layout>
  </div>
}
